<?php
/**
 * Template Name: Home Template
 */
 ?>

<?php get_header(); ?>

<div class="hero-contain" style="background:url(<?php echo the_cfc_field('page_layout', 'hero-image'); ?>);background-size:cover;">
	<div class="overlay">
		<p>Long Term Planning for the Future of Your Embryos.</p>
	</div>
	<!-- <h1 class="callout"><?php echo get_cfc_field('feature', 'feature-lead'); ?></h1> -->
</div>

	<div class="feature" style="">
<p class="regular"><?php echo get_cfc_field('feature', 'feature-content'); ?></p>
</div>
</div>

<div class="info-contain u-cf">
	<div class="four columns">
		<div class="wrapper">
			<!--<img src="<?php echo get_template_directory_uri(); ?>/css/img/ept-heart.png">-->
			<a id="spin" href="/patients">
<svg id="Layer_5" data-name="Layer 5" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="174.76" height="174.76" viewBox="0 0 174.76 174.76"><defs><style>.cls-1{fill:url(#linear-gradient-patient);}.cls-2{fill:url(#linear-gradient-2-patient);}.cls-3{fill:#008ed3;}.cls-4{fill:#a2238d;}.cls-5{fill:#562d84;}</style><linearGradient id="linear-gradient-patient" x1="1365.33" y1="-350.15" x2="1517.59" y2="-350.15" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#b31e8d"/><stop offset="1" stop-color="#e0deef"/></linearGradient><linearGradient id="linear-gradient-2-patient" x1="1388.66" y1="-372.96" x2="1540.09" y2="-372.96" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#e0deef"/><stop offset="1" stop-color="#0094d9"/></linearGradient></defs><title>ept-layout</title><g class="spin-1"><path class="cls-1" d="M1452.47-281.55a80.24,80.24,0,0,1-80.24-80.24,80.09,80.09,0,0,1,31.33-63.59,11.6,11.6,0,0,0-12.69,2.5,86.63,86.63,0,0,0-25.54,61.66,86.63,86.63,0,0,0,25.54,61.66A86.63,86.63,0,0,0,1452.53-274a86.63,86.63,0,0,0,61.66-25.54,11.61,11.61,0,0,0,2.08-13.59A80.1,80.1,0,0,1,1452.47-281.55Z" transform="translate(-1365.33 448.78)"/><path class="cls-2" d="M1514.55-423.24a86.63,86.63,0,0,0-61.66-25.54,86.63,86.63,0,0,0-61.66,25.54c-0.48.48-2.6,2.47-2.56,3,0.13,1.86,2.57,2.61,4.43,4.5A80,80,0,0,1,1452.47-442a80.24,80.24,0,0,1,80.24,80.24,80,80,0,0,1-26.08,59.2c2.09,2.39,2.88,6,4.93,5.36a25.42,25.42,0,0,0,3-2.68,86.63,86.63,0,0,0,25.54-61.66A86.63,86.63,0,0,0,1514.55-423.24Z" transform="translate(-1365.33 448.78)"/><circle class="cls-3" cx="1509.12" cy="-299.85" r="3.73" transform="translate(-711.29 1428.06) rotate(-45)"/><circle class="cls-4" cx="1390.83" cy="-418.12" r="3.27" transform="translate(-662.31 1309.78) rotate(-45)"/></g><path class="cls-5" d="M1452.68-305.52a3,3,0,0,1-4.17.64l-7.48-5.5a186.32,186.32,0,0,1-32-30.46c-12.53-15.57-11.23-29.48-10.8-34,0-.35.06-0.62,0.07-0.81-0.26-2.53,1.48-7.92,4-12.23a29.08,29.08,0,0,1,20.09-14.28c13-2.46,22.53,3.33,27.7,7.83a3,3,0,0,0,3.92,0c6.19-5.46,18.9-12.54,37.91-3.58l0.15,0.08c8.32,4.52,21.49,19.29,11.81,42.85-11.51,28-34.82,40.28-35.8,40.79h0a3,3,0,0,1-2.93-.11h0c-1.24-.76-29.8-18.47-48.47-44.94a3,3,0,0,1,.8-4.17h0a3,3,0,0,1,4.11.79c14.49,20.49,35.7,35.89,43.58,41.21a3,3,0,0,0,3.28,0c6.52-4.25,21.66-15.82,29.9-35.87,9.58-23.32-6.8-34-8.94-35.23l-0.28-.15c-22.87-10.63-34.05,5.66-34.52,6.36h0a3,3,0,0,1-4.88.12h0c-0.4-.53-10-13.18-26.22-10.14-14.44,2.72-18.75,16.33-19.24,19.64a3.16,3.16,0,0,0,0,.65,11.88,11.88,0,0,1-.09,1.72c-0.37,4-1.5,16.06,9.51,29.74,13.43,16.7,30.67,29.25,30.84,29.37l7.5,5.51a3,3,0,0,1,.64,4.17h0Zm-54.35-70.12a2.53,2.53,0,0,0,.12.57A3,3,0,0,1,1398.33-375.64Zm5.92-.59h0Zm-0.13-.72h0Z" transform="translate(-1365.33 448.78)"/></svg>

			<p class="lead">Patients</p>
			<p class="info">Protect your frozen embryos, exactly as you wish.</p>
			</a>
		</div>
	</div>
	<div class="four columns">
		<div class="wrapper">
			<a id="spin-2" href="/physicians">
<svg id="Layer_4" data-name="Layer 4" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="174.76" height="174.76" viewBox="0 0 174.76 174.76"><defs><style>.cls-1-doctors{fill:url(#linear-gradient-doc);}.cls-2-doctors{fill:url(#linear-gradient-2-doc);}.cls-3-doctors{fill:#008ed3;}.cls-4-doctors{fill:#a2238d;}.cls-5-doctors{fill:#562d84;stroke:#562d84;stroke-miterlimit:10;stroke-width:2.23px;}</style><linearGradient id="linear-gradient-doc" x1="838.23" y1="-45.26" x2="990.49" y2="-45.26" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#b31e8d"/><stop offset="1" stop-color="#e0deef"/></linearGradient><linearGradient id="linear-gradient-2-doc" x1="861.56" y1="-68.08" x2="1012.99" y2="-68.08" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#e0deef"/><stop offset="1" stop-color="#0094d9"/></linearGradient></defs><title>ept-layout</title><g class="spin-2"><path class="cls-1-doctors" d="M925.37,23.34A80.24,80.24,0,0,1,845.13-56.9a80.09,80.09,0,0,1,31.33-63.59,11.6,11.6,0,0,0-12.69,2.5,86.63,86.63,0,0,0-25.54,61.66A86.63,86.63,0,0,0,863.77,5.33a86.63,86.63,0,0,0,61.66,25.54A86.63,86.63,0,0,0,987.1,5.33a11.61,11.61,0,0,0,2.08-13.59A80.1,80.1,0,0,1,925.37,23.34Z" transform="translate(-838.23 143.89)"/><path class="cls-2-doctors" d="M987.45-118.35a86.63,86.63,0,0,0-61.66-25.54,86.63,86.63,0,0,0-61.66,25.54c-0.48.48-2.6,2.47-2.56,3,0.13,1.86,2.57,2.61,4.43,4.5a80,80,0,0,1,59.38-26.28,80.24,80.24,0,0,1,80.24,80.24A80,80,0,0,1,979.53,2.3c2.09,2.39,2.88,6,4.93,5.36a25.42,25.42,0,0,0,3-2.68A86.63,86.63,0,0,0,1013-56.69,86.63,86.63,0,0,0,987.45-118.35Z" transform="translate(-838.23 143.89)"/><circle class="cls-3-doctors" cx="982.02" cy="5.03" r="3.73" transform="translate(-554.16 839.76) rotate(-45)"/><circle class="cls-4-doctors" cx="863.73" cy="-113.23" r="3.27" transform="translate(-505.18 721.47) rotate(-45)"/></g><path class="cls-5-doctors" d="M926.21-120.8" transform="translate(-838.23 143.89)"/><path class="cls-5-doctors" d="M937.13-75.74h0a1.4,1.4,0,0,0,.55,1.85c8.33,4.77,8.09,11.25,8.08,11.53v0.09c0,7.7-8,14.06-18.25,14.9l0.7-41.73a16.46,16.46,0,0,0,14-17.73,16.54,16.54,0,0,0-15.53-14.94,16.46,16.46,0,0,0-17.28,16.42,16.46,16.46,0,0,0,14,16.26v41.77c-10.47-.68-18.72-7.12-18.72-14.95A13.6,13.6,0,0,1,912.58-74a1.4,1.4,0,0,0,.7-1.77h0a1.42,1.42,0,0,0-1.93-.76,16.56,16.56,0,0,0-9.51,14.21c0,7.53,6.17,14,14.85,16.58-6.42,2.26-10.89,7.21-10.89,12.94,0,7.24,7.12,13.23,16.31,14.13-5.92,1.07-10.34,5.13-10.34,10,0,5.18,5.06,9.47,11.61,10.15v9.75a1.57,1.57,0,0,0,1.57,1.57h0a1.57,1.57,0,0,0,1.57-1.55l0.16-9.74c6.72-.56,12-4.91,12-10.19,0-5-4.72-9.17-10.94-10.07,8.77-1.17,15.46-7,15.46-14,0-5.55-4.18-10.36-10.26-12.71,9.1-2.43,15.64-9,15.66-16.77,0-.81.13-8.59-9.46-14.09A1.42,1.42,0,0,0,937.13-75.74ZM923.37-.13c-5.65-.65-10-4.22-10-8.52s4.34-7.88,10-8.52v17Zm0-20.66c-8.55-.42-15.32-5.62-15.32-12,0-6,6.14-11,14.09-11.87l1.23,0.1v23.73Zm4.28-70.5a14.21,14.21,0,0,1-15.92-15.92A14.23,14.23,0,0,1,924-119.43a14.21,14.21,0,0,1,15.92,15.92A14.23,14.23,0,0,1,927.65-91.29ZM937-8.65c0,4.39-4.51,8-10.32,8.56L927-17.18C932.67-16.55,937-13,937-8.65Zm3.89-24.1c0,6-6,10.94-13.86,11.84l0.4-23.64C935.1-43.51,940.92-38.62,940.92-32.75Z" transform="translate(-838.23 143.89)"/></svg>
			<p class="lead">Physicians</p>
			<p class="info">Provide peace of mind to patients and your fertility practice.</p>
			</a>
		</div>
	</div>
	<div class="four columns">
		<div class="wrapper">
			<a id="spin-3" href="/cryobanks">
<!-- Generator: Adobe Illustrator 19.2.1, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
<svg version="1.1" id="Layer_4" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	 width="174.8px" height="174.8px" viewBox="0 0 174.8 174.8" style="enable-background:new 0 0 174.8 174.8;" xml:space="preserve"
	>
<style type="text/css">
	.st0{fill:url(#SVGID_1_);}
	.st1{fill:url(#SVGID_2_);}
	.st2{fill:#008ED3;}
	.st3{fill:#A2238D;}
	.st4{fill:none;stroke:#562D84;stroke-width:4.54;stroke-miterlimit:10;}
</style>
<title>ept-cryo</title>
<linearGradient id="SVGID_1_" gradientUnits="userSpaceOnUse" x1="3261.2795" y1="-454.5092" x2="3413.5417" y2="-454.5092" gradientTransform="matrix(1 0 0 -1 -3261.28 -355.88)">
	<stop  offset="0" style="stop-color:#B31E8D"/>
	<stop  offset="1" style="stop-color:#E0DEEF"/>
</linearGradient>
<g class="spin-3">
<path class="st0" d="M87.1,167.2c-44.3,0-80.2-35.9-80.2-80.2c0-24.9,11.6-48.4,31.3-63.6c-4.3-1.8-9.4-0.8-12.7,2.5
	c-34.1,34.1-34.1,89.3,0,123.3c34.1,34.1,89.3,34.1,123.3,0c3.6-3.6,4.4-9.1,2.1-13.6C135.8,155.6,112.2,167.2,87.1,167.2z"/>
<linearGradient id="SVGID_2_" gradientUnits="userSpaceOnUse" x1="3284.6094" y1="-431.6971" x2="3436.0505" y2="-431.6971" gradientTransform="matrix(1 0 0 -1 -3261.28 -355.88)">
	<stop  offset="0" style="stop-color:#E0DEEF"/>
	<stop  offset="1" style="stop-color:#0094D9"/>
</linearGradient>
<path class="st1" d="M149.2,25.5c-34.1-34.1-89.3-34.1-123.3,0c0,0,0,0,0,0c-0.5,0.5-2.6,2.5-2.6,3c0.1,1.9,2.6,2.6,4.4,4.5
	c29.8-32.8,80.5-35.3,113.3-5.5c32.8,29.8,35.3,80.5,5.5,113.3c-1.7,1.9-3.5,3.7-5.3,5.4c2.1,2.4,2.9,6,4.9,5.4
	c1.1-0.8,2.1-1.7,3-2.7C183.3,114.8,183.3,59.6,149.2,25.5C149.2,25.6,149.2,25.5,149.2,25.5z"/>

<ellipse transform="matrix(0.7071 -0.7071 0.7071 0.7071 -63.1896 145.293)" class="st2" cx="143.8" cy="148.9" rx="3.7" ry="3.7"/>
<ellipse transform="matrix(0.7071 -0.7071 0.7071 0.7071 -14.2132 27.009)" class="st3" cx="25.5" cy="30.7" rx="3.3" ry="3.3"/>
</g>
<ellipse transform="matrix(0.9125 -0.4091 0.4091 0.9125 -32.9412 29.381)" class="st4" cx="52.2" cy="91.7" rx="22.4" ry="22.4"/>
<ellipse transform="matrix(0.9125 -0.4091 0.4091 0.9125 -41.7525 40.1782)" class="st4" cx="73" cy="117.7" rx="22.4" ry="22.4"/>
<ellipse transform="matrix(0.9125 -0.4091 0.4091 0.9125 -37.5422 53.9185)" class="st4" cx="107.3" cy="114.7" rx="22.4" ry="22.4"/>
<ellipse transform="matrix(0.9125 -0.4091 0.4091 0.9125 -18.5379 32.356)" class="st4" cx="66.4" cy="59.5" rx="22.4" ry="22.4"/>
<ellipse transform="matrix(0.9125 -0.4091 0.4091 0.9125 -13.9388 45.5557)" class="st4" cx="99.5" cy="55.4" rx="22.4" ry="22.4"/>
<ellipse transform="matrix(0.9125 -0.4091 0.4091 0.9125 -23.207 56.5136)" class="st4" cx="120.5" cy="82.5" rx="22.4" ry="22.4"/>
</svg>

			<p class="lead">Cryobanks</p>
			<p class="info">Long term planning for the future of preserved embryos.</p>
			</a>
		</div>
	</div>
</div>

<div class="bio-feature u-cf">
	<div class="three columns">
		<img src="<?php echo get_template_directory_uri(); ?>/css/img/ashley-pittman.jpg">
	</div>
	<div class="nine columns">
		<div class="wrapper">
			<p>Ashley W. Pittman has combined her expertise in tax law as well as fertility law in order to help families who have struggled with infertility provide better protection for their remaining frozen embryos.  Patients often struggle with difficult decisions surrounding maintenance, donation, and disposition of their embryos which they hold precious.  Through her tax and fertility law practice, Ashley has been able to identify the issues most important to patients, and address them in the most beneficial way from a legal standpoint. </p>
			<a href="ashley-w-pittman"><p>LEARN MORE ABOUT ASHLEY W. PITTMAN</p></a>
		</div>
	</div>

</div>

<div class="cta-feature">
	<a href="/get-started"><button class="btn-primary">Start tailoring your trust today<span><img src="<?php echo get_template_directory_uri(); ?>/css/img/ept-button.png"></span></button></a>

</div>

<?php get_footer(); ?>
